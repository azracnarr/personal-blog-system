...
export default function AdminPage() {
  ...
  async function removeMessage(id: number) { try { const response = await apiFetch(`/api/contact/${id}`, { method: "DELETE", headers: { "X-CSRF-TOKEN": csrf() } }); if (!response.ok) throw new Error((await response.text()).trim() || "Mesaj silinemedi."); setMessages(current => current.filter(message => message.id !== id)); setOpenMessage(null); setStatus("Mesaj silindi."); } catch (error) { setStatus(`Mesaj silinemedi: ${error instanceof Error ? error.message : "Sunucu hatası"}`); } }
