...
export default function AdminPage() {
  ...
  async function saveAbout(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const data = Object.fromEntries(new FormData(event.currentTarget)); try { setAbout(await apiJson<About>("/api/about", { method: "PUT", headers: { "X-CSRF-TOKEN": csrf() }, body: JSON.stringify(data) })); setStatus("Hakkımda güncellendi."); } catch { setStatus("Hakkımda kaydedilemedi."); } }
  ...
  return <main className="shell"><p className="eyebrow">Yönetim paneli</p><h1>İçerik yönetimi</h1>{status && <p className="notice">{status}</p>}
    ...
    <section><h2>Gelen mesajlar</h2><div className="admin-list">{messages.map(item => <div key={item.id}><span><strong>{item.name}</strong> ({item.email})<br />{item.message}</span></div>)}</div>{!messages.length && <p className="muted">Mesaj yok.</p>}</section>
  </main>;
}
