...
export default function ContactPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try { await apiJson("/api/contact", { method: "POST", body: JSON.stringify(Object.fromEntries(form)) }); setMessage("Mesajınız gönderildi. En kısa sürede dönüş yapacağım."); event.currentTarget.reset(); } catch (error) { setMessage(error instanceof Error ? `Mesaj gönderilemedi: ${error.message}` : "Mesaj gönderilemedi."); }
  }
